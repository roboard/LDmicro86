...
static LangTable LangEsTable[] = {
    ...
    { "Compile successful; wrote IHEX for AVR to '%s'.\r\n\r\nRemember to set the processor configuration (fuses) correctly. This does not happen automatically.", "Compilación correcta; se escribi?IHEX para AVR en '%s'.\r\n\r\nRecuerde marcar la configuración (fuses) del micro correctamente. Esto NO se hace automaticamente." },
    { "( ) Normal", "( ) Normal" },
    { "(/) Negated", "(/) Negado" },
    ...
    { "--add comment here--", "--añade el comentario aqu?-" },
...
static LangTable LangFrTable[] = {
    { "Target frequency %d Hz, closest achievable is %d Hz (warning, >5%% error).", "Fréquence de la cible %d Hz, fonction accomplie ?%d Hz (ATTENTION, >5% erreur)." },
    { "Compile successful; wrote IHEX for AVR to '%s'.\r\n\r\nRemember to set the processor configuration (fuses) correctly. This does not happen automatically.", "Compil?avec succès. Ecriture du fichier IHEX pour AVR sous '%s'.\r\n\r\nVous devez configurer manuellement les Bits de configuration (fusibles). Ceci n'est pas accompli automatiquement." },
    { "( ) Normal", "( ) Normal" },
    { "(/) Negated", "(/) Inversée" },
    ...
    { "Contacts", "Contacts" },
    { "No ADC or ADC not supported for selected micro.", "Pas de convertisseur A/D ou convertisseur non support?pour le MCU sélectionn?" },
    { "Assign:", "Affectations:" },
    { "No microcontroller has been selected. You must select a microcontroller before you can assign I/O pins.\r\n\r\nSelect a microcontroller under the Settings menu and try again.", "Aucun microcontrolleur sélectionn? Vous devez sélectionner un microcontroleur avant de définir l'utilisation des broches.\r\n\r\nSélectionnez un micro dans le menu et essayez ?nouveau." },
    { "I/O Pin Assignment", "Affectation broches E/S" },
    { "Can't specify I/O assignment for ANSI C target; compile and see comments in generated source code.", "Ne pas spécifier les E/S pour code en ANSI C; Compiler et voir les commentaires dans le code source génér?" },
    { "Can't specify I/O assignment for interpretable target; see comments in reference implementation of interpreter.", "Ne pas spécifier les E/S pour sortie en code interprét? Voir les commentaires pour l'implémentation de l'interpréteur ." },
    ...
    { "The program has changed since it was last saved.\r\n\r\nDo you want to save the changes?", "Le programme a chang?depuis la dernière sauvegarde.\r\n\r\nVoulez-vous sauvegarder les changements?" },
    ...
    { "Insert EQU (Compare for Equals)\t=", "Insérer EQU (Compare pour égalit?\t=" },
    { "Insert NEQ (Compare for Not Equals)", "Insérer NEQ (Compare pour inégalit?" },
    { "Insert GRT (Compare for Greater Than)\t>", "Insérer GRT (Compare plus grand que)\t>" },
    { "Insert GEQ (Compare for Greater Than or Equal)\t.", "Insérer GEQ (Compare plus grand ou égal ?\t." },
    { "Insert LES (Compare for Less Than)\t<", "Insérer LES (Compare plus petit que)\t<" },
    { "Insert LEQ (Compare for Less Than or Equal)\t,", "Insérer LEQ (Compare plus petit ou égal ?\t," },
    ...
    { "Insert R&ES (Counter/RTO Reset)\tE", "Insérer R&ES (Remise ?zéro RTO/compteur)\tE" },
    ...
    { "Insert Shift Register", "Insérer registre ?décalage" },
    { "Insert Look-Up Table", "Insérer tableau index? },
    ...
    { "&Compile", "&Compilation" },
    { "&Help", "&Aide" },
    { "no MCU selected", "pas de MCU sélectionn? },
    ...
    { "Compile successful; wrote IHEX for PIC16 to '%s'.\r\n\r\nConfiguration word (fuses) has been set for crystal oscillator, BOD enabled, LVP disabled, PWRT enabled, all code protection off.\r\n\r\nUsed %d/%d words of program flash (chip %d%% full).", "Compil?avec succès; Ecriture IHEX pour PIC16 sous '%s'.\r\n\r\nLes bits de configuration (fuse) : Oscillateur quartz, BOD activ? LVP déactiv? PWRT activ? sans code de protection.\r\n\r\nUtilise %d/%d mots du programme flash (Chip %d%% au total)." },
    { "Type", "Type" },
    { "Timer", "Temporisation" },
    { "Counter", "Compteur" },
    { "Reset", "RAZ" },
    ...
    { "Delay cannot be zero or negative.", "Tempo ne peut être ?ZERO ou NEGATIF." },
    { "Count Up", "Compteur" },
    ...
    { "If Greater Than or Equal To", "Si plus grand ou égal ? },
    { "If Less Than", "Si plus petit que" },
    { "If Less Than or Equal To", "Si plus petit ou égal ? },
    { "'Closed' if:", "'Ferm? si:" },
    { "Move", "Mouvoir" },
    ...
    { "Name:", "Nom:" },
    { "Stages:", "Etapes:" },
    { "Shift Register", "Registre ?décalage" },
    { "Not a reasonable size for a shift register.", "N'est pas une bonne taille pour un registre ?décalage." },
    { "String:", "Chaine:" },
    ...
    { "Compile successful; wrote C source code to '%s'.\r\n\r\nThis is not a complete C program. You have to provide the runtime and all the I/O routines. See the comments in the source code for information about how to do this.", "Compil?avec succès; Le code source C enregistr?sous '%s'.\r\n\r\nCe programme n'est pas complet. Vous devez prévoir le runtime ainsi que toutes les routines d'entrées/sorties. Voir les commentaires pour connaitre la façon de procéder." },
    ...
    { "Internal relay '%s' never assigned; add its coil somewhere.", "Relais internes '%s', jamais utilisés, ?utiliser pour la commande de bobines dans le programme." },
    { "Must assign pins for all I/O.\r\n\r\n'%s' is not assigned.", "Vous devez spécifier les broches pour toutes les E/S.\r\n\r\n'%s' ." },
    { "UART in use; pins %d and %d reserved for that.", "UART utilis? broches %d et %d réservée pour cette fonction." },
    { "PWM in use; pin %d reserved for that.", "PWM utilis? broche %d réservée pour cela." },
    { "UART baud rate generator: divisor=%d actual=%.4f for %.2f%% error.\r\n\r\nThis is too large; try a different baud rate (slower probably), or a crystal frequency chosen to be divisible by many common baud rates (e.g. 3.6864 MHz, 14.7456 MHz).\r\n\r\nCode will be generated anyways but serial may be unreliable or completely broken.", "UART Générateur vitesse: Diviseur=%d actuel=%.4f pour %.2f%% Erreur.\r\n\r\nCeci est trop important; Essayez une autre vitesse (probablement plus lente), ou choisir un quartz divisible par plus de vitesses de transmission (comme 3.6864MHz, 14.7456MHz).\r\n\r\nCode est tout de même génér? mais la liaison peut être inutilisable ou la transmission erronée." },
    { "UART baud rate generator: too slow, divisor overflows. Use a slower crystal or a faster baud rate.\r\n\r\nCode will be generated anyways but serial will likely be completely broken.", "Générateur vitesse UART trop lent, dépassement de capacit?du diviseur. Utiliser un quartz plus lent ou une vitesse plus élevée.\r\n\r\nCode est génér?mais la liaison semble inutilisable." },
    ...
    { "Piecewise linear lookup table with zero elements!", "Le tableau index?linéaire ne comporte aucun élément!" },
    { "x values in piecewise linear table must be strictly increasing.", "Les valeurs x du tableau index?linéaire doivent être obligatoirement dans un ordre croissant." },
    ...
    { "No variable is interpolated into formatted string, but a variable name is specified. Include a string like '\\-3', or leave variable name blank.", "Aucune variable n'est appelée dans une chaine formattée, mais un nom de variable est spécifi?. Insérer une chaine comme: '\\-3', ou laisser le nom de variable vide." },
    ...
    { "Unsupported op (anything ADC, PWM, UART, EEPROM) for interpretable target.", "Instructions non supportées (comme ADC, PWM, UART, EEPROM ) pour sortie en code interprét?" },
    { "Compile successful; wrote interpretable code to '%s'.\r\n\r\nYou probably have to adapt the interpreter to your application. See the documentation.", "Compil?avec succès; Code interprét?écrit sous '%s'.\r\n\r\nQue vous devez adapter probablement pour votre application. Voir documentation." },
    { "Microcontroller '%s' not supported.\r\n\r\nDefaulting to no selected MCU.", "Microcontrolleur '%s' non support?\r\n\r\nErreur pas de MCU sélectionn?" },
    ...
    { "Compile Successful", "Compil?avec succès" },
...
static LangTable LangPtTable[] = {
    { "Target frequency %d Hz, closest achievable is %d Hz (warning, >5%% error).", "Freqüência do Micro %d Hz, a melhor aproximação ?%d Hz (aviso, >5%% error)." },
    ...
    { "No serial instructions (UART Send/UART Receive) are in use; add one to program before setting baud rate.\r\n\r\n", "Nenhuma instrução serial (UART Enviar/UART Recebe) est?em uso; adicione uma ao programa antes de configurar a taxa de transmissão (baud rate).\r\n\r\n" },
    { "The cycle time for the 'PLC' runtime generated by LDmicro is user-configurable. Very short cycle times may not be achievable due to processor speed constraints, and very long cycle times may not be achievable due to hardware overflows. Cycle times between 10 ms and 100 ms will usually be practical.\r\n\r\nThe compiler must know what speed crystal you are using with the micro to convert between timing in clock cycles and timing in seconds. A 4 MHz to 20 MHz crystal is typical; check the speed grade of the part you are using to determine the maximum allowable clock speed before choosing a crystal.", "O tempo de ciclo para o PLC generalizado pelo LDmicro ?configuravel pelo usuário. Tempos de ciclo muito pequenos podem não ser realizáveis devido a baixa velocidade do processador, e tempos de ciclo muito longos podem não ser realizáveis devido ao temporizador do micro. Ciclos de tempo entre 10 e 100ms são mais usuais.\r\n\r\n O compilador tem que saber qual ?a freqüência do cristal que est?sendo usado para poder converter entre o tempo em ciclos do clock e  tempo em segundos. Cristais entre 4 Mhz e 20 Mhz são os mais típicos. Confira a velocidade  que pode funcionar seu micro e calcule a velocidade máxima do clock antes de encolher o cristal." },
    { "PLC Configuration", "Configuração PLC" },
    { "Zero cycle time not valid; resetting to 10 ms.", "Não ?válido um tempo de ciclo 0; retornando a 10 ms." },
    { "Source", "Fonte" },
    { "Internal Relay", "Rele Interno" },
    ...
    { "No ADC or ADC not supported for this micro.", "Este micro não tem ADC ou não ?suportado." },
    ...
    { "UART function used but not supported for this micro.", "Função UART ?usada porem não suportada por este micro." },
    { "PWM function used but not supported for this micro.", "Função PWM ?usada porem não suportada por este micro." },
    { "The program has changed since it was last saved.\r\n\r\nDo you want to save the changes?", "Este programa contem mudanças desde a ultima vez salva.\r\n\r\n Voc?quer salvar as mudanças?" },
    ...
    { "Zero baud rate not possible.", "Zero Baud Rate não ?possível." },
    ...
    { "OK", "OK" },
    { "Cancel", "Cancelar" },
    { "Empty textbox; not permitted.", "Texto vazio; não ?permitido" },
    ...
    { "Not a reasonable size for a shift register.", "Não ?um tamanho razoável para um registro de deslocamento." },
    { "String:", "String:" },
    ...
    { "UART baud rate generator: divisor=%d actual=%.4f for %.2f%% error.\r\n\r\nThis is too large; try a different baud rate (slower probably), or a crystal frequency chosen to be divisible by many common baud rates (e.g. 3.6864 MHz, 14.7456 MHz).\r\n\r\nCode will be generated anyways but serial may be unreliable or completely broken.", "Gerador UART baud rate: divisor=%d atual=%.4f para %.2f%% error.\r\n\r\nEste ?muito grande; Tente com outro valor (provavelmente menor), ou um cristal cuja freqüência seja divisível pelos baud rate mais comuns (p.e. 3.6864MHz, 14.7456MHz).\r\n\r\nO código ser?gerado de qualquer maneira, porem a serial poder?ser incerta ou completamente fragmentada." },
    ...
    { "Move instruction: '%s' not a valid destination.", "Instrução Mover: '%s' o destino não ?válido." },
    { "Math instruction: '%s' not a valid destination.", "Instruções Math: '%s' o destino não ?válido." },
    ...
    { "Multiple escapes (\\0-9) present in format string, not allowed.", "Não ?permitido mais de um caractere especial (\\0-9) dentro da string formatada." },
    { "Bad escape: correct form is \\xAB.", "Caractere Especial com Erro: A forma correta ?\\xAB." },
    { "Bad escape '\\%c'", "Caractere Especial com Erro '\\%c'" },
    { "Variable is interpolated into formatted string, but none is specified.", "A variável ?interpolada dentro da string formatada, mas nenhuma ?especificado." },
    { "No variable is interpolated into formatted string, but a variable name is specified. Include a string like '\\-3', or leave variable name blank.", "Nenhuma variável esta interpolada dentro da string formatada, porem um nome de variável ?especificada. Inclua uma string como '\\-3', ou deixe o nome da variável em branco." },
    ...
    { "Compile successful; wrote interpretable code to '%s'.\r\n\r\nYou probably have to adapt the interpreter to your application. See the documentation.", "Compilação sucedida: Código para interpretador escrito para '%s'.\r\n\r\nVoc?provavelmente tem que adaptar o interpretador para sua aplicação. Veja a documentação." },
    { "Microcontroller '%s' not supported.\r\n\r\nDefaulting to no selected MCU.", "Microcontrolador '%s' não suportado.\r\n\r\nFalha nenhum Micro selecionado." },
    { "File format error; perhaps this program is for a newer version of LDmicro?", "Erro no formato de arquivo; talvez este programa ?para uma versão mais nova do LDmicro?." },
    { "Index:", "Índice:" },
    { "Points:", "Pontos:" },
    { "Count:", "Contador:" },
...
static LangTable LangTrTable[] = {
    { "Target frequency %d Hz, closest achievable is %d Hz (warning, >5%% error).", "Hedef frekans %d Hz, bu deðere en yakýn olas?deðer %d Hz (Uyar? >5% hatas?." },
    { "Compile successful; wrote IHEX for AVR to '%s'.\r\n\r\nRemember to set the processor configuration (fuses) correctly. This does not happen automatically.", "Derleme baþarýl? AVR için derlenen '%s' IHEX dosyasýna kaydedildi.\r\n\r\n MÝB konfigürasyonunu ayarlamay?unutmayýn. Bu iþlem otomatik olarak yapýlmamaktadýr." },
    { "( ) Normal", "( ) Normal" },
    { "(/) Negated", "(/) Terslenmi? },
    { "(S) Set-Only", "(S) Set" },
    ...
    { "UART Baud Rate (bps):", "UART Baud Rate (bps):" },
    { "Serial (UART) will use pins %d and %d.\r\n\r\n", "Seri iletiþim için (UART) %d ve %d bacaklar?kullanýlacaktýr.\r\n\r\n" },
    { "Please select a micro with a UART.\r\n\r\n", "Lütfen UART olan bir MÝB seçiniz.\r\n\r\n" },
    { "No serial instructions (UART Send/UART Receive) are in use; add one to program before setting baud rate.\r\n\r\n", "Seri iletiþim komutu kullanmadýnýz. Hýz?ayarlamadan önce komut kullanmalýsýnýz.\r\n\r\n" },
    { "The cycle time for the 'PLC' runtime generated by LDmicro is user-configurable. Very short cycle times may not be achievable due to processor speed constraints, and very long cycle times may not be achievable due to hardware overflows. Cycle times between 10 ms and 100 ms will usually be practical.\r\n\r\nThe compiler must know what speed crystal you are using with the micro to convert between timing in clock cycles and timing in seconds. A 4 MHz to 20 MHz crystal is typical; check the speed grade of the part you are using to determine the maximum allowable clock speed before choosing a crystal.", "PLC için LDMicro tarafýndan verilen çevrim süresi kullanýc?tarafýndan deðiþtirilebilir. Çok kýsa süreler iþlemcinin hýzýndan kaynaklanan sebeplerden dolay?verilemeyebilir. 10ms ile 100ms arasýndaki deðerler çevrim süresi için genellikle uygun deðerlerdir.\r\n\r\n.LDMicro kullanýlan kristalin hýzýn?bilmelidir. Bu deðer program içerisinde kullanýlýr. Bilindiði üzere genellikle 4MHz ile 20MHz arasýnda deðere sahip kristaller kullanýlmaktadýr. Kullandýðýnýz kristalin deðerini ayarlamalýsýnýz." },
    ...
    { "Input pin", "Giri?Bacaðý" },
    { "Output pin", "Çýkýþ Bacaðý" },
    { "|/| Negated", "|/| Terslenmi? },
    { "Contacts", "Kontak" },
    { "No ADC or ADC not supported for selected micro.", "Bu iþlemcide ADC yok yada ADC desteklenmiyor." },
    { "Assign:", "Deðeri:" },
    { "No microcontroller has been selected. You must select a microcontroller before you can assign I/O pins.\r\n\r\nSelect a microcontroller under the Settings menu and try again.", "Ýþlemci seçmediniz.\r\n\r\n G/?uçlarýn?seçmeden önce Ayarlar menüsünden iþlemci seçiniz." },
    { "I/O Pin Assignment", "G/?Bacak Taným? },
    { "Can't specify I/O assignment for ANSI C target; compile and see comments in generated source code.", "ANSI C hedef için G/?tanýmlamas?yapýlamad? Dosyay?derleyerek oluþturulan kaynak kodundaki yorumlar?inceleyiniz." },
    { "Can't specify I/O assignment for interpretable target; see comments in reference implementation of interpreter.", "Hedef için G/?taným?yapýlamad? Derleyicinin kullaným kitapçýðýn?inceleyiniz." },
    { "Can only assign pin number to input/output pins (Xname or Yname or Aname).", "Sadece giri?çýkýþ uçlar?için bacak tanýmlamas?yapýlabilir. (XName veya YName veya AName)." },
    { "No ADC or ADC not supported for this micro.", "Bu iþlemcide ADC yok yada ADC desteklenmiyor." },
    { "Rename I/O from default name ('%s') before assigning MCU pin.", "('%s') öntanýml?isimdir. MÝB bacaðý tanýmlamadan önce bu ismi deðiþtiriniz." },
    { "I/O Pin", "G/?Bacaklar? },
    { "(no pin)", "(Bacak Yok)" },
    ...
    { "Couldn't write to '%s'.", "'%s' dosyasýna yazýlamad?" },
    ...
    { "LDmicro - Program Editor", "LDmicro ?Program Editör?" },
    ...
    { "Move Selected Rung &Up\tShift+Up", "Seçili satýr?üste taþý\tShift+Up" },
    { "Move Selected Rung &Down\tShift+Down", "Seçili satýr?alta taþý\tShift+Down" },
    { "&Delete Selected Element\tDel", "&Seçili Eleman?Sil\tDel" },
    { "D&elete Rung\tShift+Del", "Seçili Satýr?Sil\tShift+Del" },
    ...
    { "Insert CT&U (Count Up)\tU", "CT&U ekle (Yukar?Sayýc?\tU" },
    { "Insert CT&D (Count Down)\tI", "CT&D ekle (Aþaðý Sayýc?\tI" },
    { "Insert CT&C (Count Circular)\tJ", "CT&C ekle (Döngüsel Sayýc?\tJ" },
    { "Insert EQU (Compare for Equals)\t=", "EQU ekle (Eþitlik Kontrol?\t=" },
    { "Insert NEQ (Compare for Not Equals)", "NEQ ekle (Farklýlýk Kontrol?" },
    { "Insert GRT (Compare for Greater Than)\t>", "GRT ekle (Büyük  Kontrol?\t>" },
    { "Insert GEQ (Compare for Greater Than or Equal)\t.", "GEQ ekle (Büyük yada Eþit Kontrol?\t." },
    { "Insert LES (Compare for Less Than)\t<", "LES ekle (Küçük Kontrol?\t<" },
    { "Insert LEQ (Compare for Less Than or Equal)\t,", "LEQ ekle (Küçük yada Eþit Kontrol?\t," },
    ...
    { "Insert R&ES (Counter/RTO Reset)\tE", "R&ES ekle (RTO/Sayýc?Sýfýrlamas?\tE" },
    ...
    { "Insert Piecewise Linear", "Parçal?Lineer ekle" },
    { "Insert Formatted String Over UART", "UART Üzerinden Biçimlendirilmi?Kelime Ekle" },
    { "Insert &UART Send", "&UART'dan Gönderme ekle" },
    { "Insert &UART Receive", "&UART'dan Alma ekle" },
    { "Insert Set PWM Output", "PWM Çýkýþ?Akit Et ekle" },
    ...
    { "processor clock %.4f MHz", "iþlemci frekans?%.4f MHz" },
    { "Internal error relating to PIC paging; make program smaller or reshuffle it.", "PIC sayfalamas?ile ilgili dahili hata oluþtu. Program?kýsaltýnýz yada deðiþtiriniz." },
    { "PWM frequency too fast.", "PWM frekans?çok hýzl?" },
    { "PWM frequency too slow.", "PWM frekans?çok yava?" },
    { "Cycle time too fast; increase cycle time, or use faster crystal.", "Çevrim süresi çok kýsa. Süreyi yada kristal frekansýn?artýrýnýz." },
    { "Cycle time too slow; decrease cycle time, or use slower crystal.", "Çevrim süresi çok uzun. Süreyi yada kristal frekansýn?azaltýnýz.." },
    { "Couldn't open file '%s'", "'%s' dosyas?açýlamad?" },
    { "Zero baud rate not possible.", "Ýletiþim hýz?(Baudrate) sýfýr olamaz." },
    { "Compile successful; wrote IHEX for PIC16 to '%s'.\r\n\r\nConfiguration word (fuses) has been set for crystal oscillator, BOD enabled, LVP disabled, PWRT enabled, all code protection off.\r\n\r\nUsed %d/%d words of program flash (chip %d%% full).", "Derleme baþarýyla yapýld? PIC16 için IHEX dosyas?'%s' dosyasýna kaydedildi.\r\n\r\nAyarlar: (PIC konfigürasyonu) kristal osilatör, BOD aktif, LVP pasif, PWRT aktif, tüm kod korumas?kapal?\r\n\r\nPIC hafýzasýnýn %d/%d kelimesi kullanýld? (Hafýza %d%% dolu)." },
    { "Type", "Tipi" },
    { "Timer", "Zamanlayýc? },
    { "Counter", "Sayýc? },
    { "Reset", "Reset" },
    ...
    { "Circular Counter", "Dairesel Sayýc? },
    ...
    { "Formatted String Over UART", "UART Üzerinden Biçimlendirilmi?Karakter Dizisi" },
    { "Variable:", "Deðiþken:" },
    { "Make Persistent", "EEPROM'da Sakla" },
    { "Too many elements in subcircuit!", "Alt devrede çok fazla eleman var!" },
    { "Too many rungs!", "Satýr sayýs?(Rung) fazla!" },
    { "Error", "Hata" },
    ...
    { "Compile successful; wrote C source code to '%s'.\r\n\r\nThis is not a complete C program. You have to provide the runtime and all the I/O routines. See the comments in the source code for information about how to do this.", "Derleme baþarýyla tamamland? C kaynak kodu '%s' dosyasýna yazýld?\r\n\r\nBu dosya tam bir C dosyas?deðildir. G/?rutinlerini kendiniz saðlamalýsýnýz. Dosyay?incelemeniz faydal?olacaktýr." },
    { "Cannot delete rung; program must have at least one rung.", "Bu satýr silinemez. Programda en az bir tane satýr (Rung) olmalýdýr." },
    { "Out of memory; simplify program or choose microcontroller with more memory.", "ÝÞlemci hafýzas?doldu; Program?kýsaltýnýz yada daha yüksek hafýzas?olan bir iþlemci seçiniz." },
    { "Must assign pins for all ADC inputs (name '%s').", "Tüm ADC giriþlerinin bacaklar?belirtilmelidir (ADC '%s')." },
    { "Internal limit exceeded (number of vars)", "Dahili sýnýr aþýld?(Deðiþken Sayýs?" },
    ...
    { "Must assign pins for all I/O.\r\n\r\n'%s' is not assigned.", "Tüm G/?uçlar?için bacaklar belirtilmelidir.\r\n\r\n'%s' ucuna deðer verilmemi?" },
    { "UART in use; pins %d and %d reserved for that.", "UART Kullanýmda; %d ve %d bacaklar?bunun için kullanýlmaktadýr. Siz kullanamazsýnýz." },
    { "PWM in use; pin %d reserved for that.", "PWM Kullanýmda; %d bacaðý bunun için ayrýlmýþtýr. Siz kullanamazsýnýz.." },
    { "UART baud rate generator: divisor=%d actual=%.4f for %.2f%% error.\r\n\r\nThis is too large; try a different baud rate (slower probably), or a crystal frequency chosen to be divisible by many common baud rates (e.g. 3.6864 MHz, 14.7456 MHz).\r\n\r\nCode will be generated anyways but serial may be unreliable or completely broken.", "UART Hýz Hesaplayýcýs? Bölen=%d Gerçekte=%.4f (%.2f%% hata pay?ile).\r\n\r\nBu deðer çok yüksektir. Deðiþik bir deðer denemelisiniz yada kristal frekansýn?sýk kullanýlan ve bu deðerlere bölünebilen bir deðer seçiniz. (Mesela 3.6864MHz, 14.7456MHz gibi).\r\n\r\nHer durumda kod oluþturulacaktýr; ancak seri iletiþim düzgün çalýþmayabilir yada tamamen durabilir." },
    { "UART baud rate generator: too slow, divisor overflows. Use a slower crystal or a faster baud rate.\r\n\r\nCode will be generated anyways but serial will likely be completely broken.", "UART Hýz Hesaplayýcýs? Hýz çok düþük. Bu nedenle bölen taþma yapýyor. Ya kristal frekansýn?düþürünüz yada hýz?(baudrate) artýrýnýz.\r\n\r\nHer durumda kod oluþturulacaktýr; ancak seri iletiþim düzgün çalýþmayabilir yada tamamen durabilir." },
    { "Couldn't open '%s'\n", "'%s' dosyas?açýlamadý\n" },
    { "Timer period too short (needs faster cycle time).", "Zamanlayýc?peryodu çok kýsa (daha yüksek çevrim süresi gerekli)." },
    { "Timer period too long (max 32767 times cycle time); use a slower cycle time.", "Zamanlayýc?peryodu çok kýsa(en fazla. çevrim süresinin 32767 kat?olabilir); çevrim süresini düþürünüz." },
    ...
    { "Piecewise linear lookup table with zero elements!", "Parçal?doðrusal arama tablosunda deðer yok!" },
    { "x values in piecewise linear table must be strictly increasing.", "Parçal?doðrusal arama tablosundaki x deðerleri artan sýralamal?olmal?" },
    { "Numerical problem with piecewise linear lookup table. Either make the table entries smaller, or space the points together more closely.\r\n\r\nSee the help file for details.", "Parçal?doðrusal arama tablosunda sayýsal hata.\r\n\r\nDetaylar için yardým menüsün?inceleyiniz." },
    ...
    { "Variable is interpolated into formatted string, but none is specified.", "Biçimlendirilmi?karakter kümesine deðiþken tanýmlanmýþ ama hi?belirtilmemi?" },
    { "No variable is interpolated into formatted string, but a variable name is specified. Include a string like '\\-3', or leave variable name blank.", "Biçimlendirilmi?karakter kümesine deðiþken atanmamýþ ama deðiþken ismi belirtilmi? Ya deðiþken ismini belirtiniz yada '\\-3' gibi bir deðer veriniz.." },
    { "Empty row; delete it or add instructions before compiling.", "Satýr bo? Satýr?silmeli yada komut eklemelisiniz." },
    ...
    { "Compile successful; wrote interpretable code to '%s'.\r\n\r\nYou probably have to adapt the interpreter to your application. See the documentation.", "Derleme baþarýyla tamamland? Kod '%s' dosyasýna yazýld?\r\n\r\nDerleyiciyi uygulamanýza göre deðiþtirmeniz gerekebilir. Geni?bilgi için LDMicro belgelerine baþvurabilirsiniz." },
    { "Microcontroller '%s' not supported.\r\n\r\nDefaulting to no selected MCU.", "'%s' desteklenen bir iþlemci deðil.\r\n\r\nÝþlemci ayar?iþlemci yok þeklinde deðiþtirilmiþtir." },
    { "File format error; perhaps this program is for a newer version of LDmicro?", "Dosya biçimi hatas? Açmaya çalýþtýðýnýz dosya ya LDMicro dosyas?deðil yada yeni bir sürüm ile yazýlmýþ." },
    { "Index:", "Liste:" },
    { "Points:", "Deðer Sayýs?" },
    { "Count:", "Adet:" },
    { "Edit table of ASCII values like a string", "ASCII deðer tablosunu karakter dizisi gibi düzenle." },
    { "Look-Up Table", "Arama Tablosu" },
    { "Piecewise Linear Table", "Parçal?Lineer Tablo" },
    { "LDmicro Error", "LDmicro Hatas? },
    { "Compile Successful", "Derleme Baþarýl? },
    ...
    { "retentive timer", "deðeri saklanan zamanlayýc? },
    { "counter", "Sayýc? },
    { "general var", "Genel Deðiþken" },
    { "adc input", "ADC Giriþi" },
    { "<corrupt!>", "<bozuk>" },
    { "(not assigned)", "(tanýml?deðil)" },
    { "<no UART!>", "<UART Yok!>" },
    { "<no PWM!>", "<PWM Yok!>" },
    ...
    { "Variable '%s' not assigned to, e.g. with a MOV statement, an ADD statement, etc.\r\n\r\nThis is probably a programming error; now it will always be zero.", "'%s' deðer atanmamýþ.Örneðin, MOV, ADD komutlar?ile deðer atanabilir.\r\n\r\nMuhtemelen bu bir programlama hatasýdýr. Bu nedenle deðiþkenin deðeri 0 olarak kullanýlacaktýr." },
    { "Variable for '%s' incorrectly assigned: %s.", "'%s' için deðiþken hatal?atanmýþ: %s." },
