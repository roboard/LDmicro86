...
char *HelpTextDe[] = {
    ...
    "hinzufügen, indem Sie ‘Netzwerk Einfügen Davor/Danach?im Programm-Men?,
    ...
    "Anweisung zum Beispiel, die eine ??in einen ‘TBewahrend?transferiert,",
    ...
    "wählt dann die nächst erreichbare Frequenz; falls der Fehler zu gro?ist,",
    ...
    "",
    "Falls Ihr Programm zu gro?für die Zeit-, Programmspeicher- oder",
...
char *HelpTextFr[] = {
    ...
    "de commandes. Ces programmes sont puissants et adaptés ?l'architecture",
    ...
    "allemand) utilisent une autre voie et sont programmés en Langage ?,
    "Contacts (ou LADDER). Un programme simple est représent?comme ceci :",
    "",
    "",
    "   ||                                                                    ||",
    "   ||    Xbutton1           Tdon           Rchatter           Yred       ||",
    ...
    "relais. Beaucoup de références de programmes de langage ?contacts (LADDER) ",
    "existent sur Internet et sont ?quelques détails près, identiques ?",
    ...
    "",
    "    * Le programme est représent?dans un format graphique, et non",
    ...
    "",
    "    * Le compilateur de langage ?contacts vérifie tout ceci lors",
    "      de la compilation. Vous n'avez pas ?écrire de code quand une",
    "      Sortie est remplacée et est remise en Entrée ou si une temporisation",
    "      est modifiée, vous n'avez pas non plus ?spécifier l'ordre o?les",
    ...
    "",
    "",
    "LDmicro est conçu pour être similaire ?la majorit?des API commerciaux.",
    ...
    "document considère que vous avez une connaisance de base du langage ?,
    ...
    "mais le runtime est de votre responsabilit? LDmicro gérére uniquement",
    ...
    "Ceci permet ?LDmicro de démarrer automatiquement lors d'un double clic",
    ...
    "et sauvegardera le fichier compil?sous`dest.hex'. Après compilation",
    ...
    "qui sera nomm?`Xnew'. `X' désigne un contact qui peut être li??une",
    "broche d'entrée du microcontroleur, vous pouvez affecter la broche pour",
    "ce contact plus tard après avoir choisi le microcontroleur et renomm?,
    "les contacts. La première lettre indique de quel type de composants il",
    "s'agit par exemple :",
    "",
    "    * Xnom  -- Reli??une broche d'entrée du microcontroleur",
    ...
    "dans le même programme, le comptage effectu?par ces tempo utilisera le",
    ...
    "majorit?des endroits o?vous pouvez utiliser les nombres décimaux.",
    "",
    ...
    "affectés ?une broche du micro La première chose ?faire est de choisir",
    "la microcontroleur utilis?: Paramères -> Microcontroleur ensuite vous",
    ...
    "pouvez insérer une nouvelle instruction ?la droite ou ?la gauche",
    ...
    "ou presser <Ctrl+M> le programme est affich?différemment en mode",
    "simulation. Les instructions activées sont affichées en rouge vif, les",
    "instructions qui ne le sont pas sont affichées en gris? Appuyer sur la",
    ...
    "en temps réel ou presser <Ctrl+R> L'affichage du programme est mise ?,
    ...
    "======================",
    "",
    "Le point final est de générer un fichier .HEX qui sera programm?dans le",
    ...
    "le temps de cycle peut être laiss??la valeur par défaut (10 ms) qui est",
    "une bonne valeur pour la majorit?des applications. Indiquer la fréquence",
    "du quartz utilis?(ou du résonateur céramique ou autres..) et cliquer OK.",
    "",
    ...
    "Si vous avez précédemment compil?votre programme, vous pouvez spécifier",
    "un nom différent de fichier de sortie. Si votre programme ne comporte",
    "pas d'erreur (li??la structure du programme), LDmicro génére un fichier",
    "IHEX prêt ?être programm?dans le chip.",
    "",
    ...
    "    interne ou la broche de sortie est faux (mise ?zéro). Si le signal",
    "    arrivant ?cette instruction est vrai(1), alors le relais interne ou",
    "    la broche de sortie est validée (mise ?1). Il n'est pas important",
    "    d'affecter une variable ?une bobine.",
    "    Cette instruction est placée le plus ?droite dans une séquence.",
    "    ",
    "",
    ...
    "    relais interne ou la broche de sortie est validée (mise ?1). Cette",
    "    instruction permet de changer l'état d'un relais ou d'une sortie :",
    "    uniquement passe ?vrai, ou reste vrai si elle était déj??1,",
    ...
    "",
    "    Si le signal arrivant ?cette instruction est vrai, alors le relais ",
    "    interne ou la sortie est mise ?zéro (0), si elle était déj??0, ",
    "    il n'y a aucun changement, cette instruction change l'état d'une ",
    "    sortie uniquement si elle était ?1, cette instruction fonctionne en ",
    ...
    "    le signal de sortie passe immédiatement ?zéro. La tempo est remise",
    "    ?zéro ?chaque fois que l'entrée repasse ?zéro. L'entrée doit être",
    "    maintenue vraie ?1 pendant au moins 1000 millisecondes consécutives",
    ...
    "    compteur est plus grande ou égale au delai fix? Il est possible",
    ...
    "                           -[TOF 1.000 s]-",
    "",
    "    Quand le signal qui arrive ?l'instruction passe de l'état vrai",
    "    (1) ?l'état faux (0), la sortie attend 1.000 s avant de dévenir",
    "    faux (0) Quand le signal arrivant ?l'instruction passe de l'état",
    "    faux ?l'état vrai, le signal passe ?vrai immédiatement. La",
    "    temporisation est remise ?zéro ?chaque fois que l'entrée devient",
    "    fausse. L'entrée doit être maintenue ?l'état faux pendant au moins",
    "    1000 ms consécutives avant que la sortie ne passe ?l'état faux. La",
    ...
    "                                   -[RTO 1.000 s]-",
    "",
    "    Cette instruction prend en compte le temps que l'entrée a ét??l'état",
    "    vrai (1). Si l'entrée a ét?vraie pendant au moins 1.000s la sortie",
    ...
    "    va devenir vraie. Après être pass??l'état vrai, la sortie reste",
    "    vraie quelque soit la commande de l'instruction. La temporisation",
    "    doit être remise ?zéro par une instruction de RES (reset).",
    "",
    "    La variable `Tnom' compte depuis zéro en unités de temps de scan.",
    "    L'instruction Ton devient vraie en sortie quand la variable du",
    "    compteur est plus grande ou égale au delai fix? Il est possible",
    ...
    "",
    "",
    "> RES Remise ?Zéro             Trto             Citems",
    "                           ----{RES}----     ----{RES}----",
    "",
    "    Cette instruction fait un remise ?zéro d'une temporisation ou d'un",
    "    compteur. Les tempos TON et TOF sont automatiquement remisent ?zéro",
    ...
    "    et les compteurs décompteurs CTU / CTD ne sont pas remis ?zéro",
    "    automatiquement, il faut donc utiliser cette instruction. Lorsque",
    "    l'entrée est vraie , le compteur ou la temporisation est remis ?,
    "    zéro. Si l'entrée reste ?zéro, aucune action n'est prise.",
    "    Cette instruction est placée le plus ?droite dans une séquence.",
    "",
    "",
    "> FRONT MONTANT                    _",
    ...
    "    génére une impulsion ?chaque front montant du signal d'entrée. Cette",
    ...
    "    vraie. Elle génére une impulsion ?chaque front descendant du signal",
    ...
    "    égale ?la condition d'entrée. Une instruction Circuit Ouvert donne ",
    ...
    "    variable de destination ?une valeur égale ?la variable source ou ?,
    ...
    "    ?une instruction de déplacement, ceci inclu l'état de variables",
    "    compteurs ou temporisateurs qui se distinguent par l'entête T ou",
    "    C. Par exemple mettre 0 dans Tsauvegard?équivaut ?faire une RES",
    "    de la temporisation.  Cette instruction doit être complétement ?,
    ...
    "    destination la variable égale ?l'expression calculée. Les opérandes",
    ...
    "    bits signées. Il faut se souvenir que le résultat est évalu??chaque",
    ...
    "    aussi une des opérandes), le résultat ne sera pas celui escompt?",
    "    il faut utiliser typiquement un front montant ou descendant de la",
    "    condition d'entrée qui ne sera évalu?qu'une seule fois. La valeur",
    "    est tronquée ?la valeur entière.  Cette instruction doit être",
    "    complétement ?droite dans une séquence.",
    "",
    "",
    "> COMPARER              [var ==]        [var >]        [1 >=]",
    "                       -[ var2 ]-      -[ 1   ]-      -[ Ton]-",
    ...
    "    est utilisée pour comparer (Egalit? plus grand que,plus grand ou",
    "    égal ? inégal, plus petit que, plus petit ou égal ? une variable ?,
    ...
    "    (Décompteur CTD, count down) une variable ?chaque front montant de",
    "    la ligne en condition d'entrée (CAD quand la signal passe de l'état",
    "    0 ?l'état 1. La condition de sortie du compteur est vraie si la",
    ...
    "    une variable.  L'instruction Remise ?Zéro permet de resetter un",
    "    compteur (remettre ?zéro), il est possible de modifier par des",
    ...
    "    limite supérieure, la variable du compteur revient ?0. dans",
    ...
    "    CTC est validée par la condition d'entrée associée ?une instruction",
    "    de front montant.",
    "    Cette instruction doit être complétement ?droite dans une séquence.",
    "    ",
    "",
    "> REGISTRE A DECALAGE       {SHIFT REG   }",
    "                           -{ reg0..3    }-",
    "",
    "    Un registre ?décalage est associ?avec un jeu de variables. Le",
    "    registre ?décalage de l'exemple donn?est associ?avec les",
    "    variables`reg0', `reg1', `reg2', and `reg3'. L'entrée du registre ?,
    "    décalage est `reg0'. A chaque front montant de la condition d'entrée",
    "    de la ligne, le registre ?décalage va décaler d'une position ?,
    "    droite. Ce qui donne `reg3 := reg2', `reg2 := reg1'. et `reg1 :=",
    "    reg0'.`reg0' est ?gauche sans changement. Un registre ?décalage",
    ...
    "    d'entrée est vraie, la variable entière `dest' est mise ?la valeur",
    ...
    "        (x3, y3)   = (100, 100)",
    "",
    "    liés ?cette courbe. Vous pouvez entrer ces 4 points dans un",
    "    tableau associ??l'instruction tableau d'éléments linéaires. Cette",
    ...
    "    points du tableau, par exemple ce tableau est équivalent ?celui ci",
    ...
    "    une broche du micro ?la variable Anom de la même façon que pour",
    ...
    "    correspond ?une lecture ADC de 0, et une valeur égale ?VDD (la",
    "    tension d'alimentation ) correspond ?une lecture ADC de 1023. Si",
    "    vous utilisez un circuit AVR, vous devez connecter Aref ?VDD.",
    "",
    "    Vous pouvez utiliser les opérations arithmétiques pour mettre ?",
    "    l'échelle les lectures effectuées dans l'unit?qui vous est la plus ",
    ...
    "    PWM va être fix??la valeur de la variable Rapport de cycle PWM.",
    ...
    "    met automatiquement ?l'échelle la variable du rapport de cycle en",
    ...
    "    de la résolution. Cette instruction doit être complétement ?droite",
    "    dans une séquence.",
    "",
    ...
    "    de l'alimentation du micro. Il n'y a pas ?spécifier ou elle doit",
    "    être sauvegardée en EEPROM, ceci est fait automatiquement, jusqu'a",
    "    ce quelle change ?nouveau.  La variable est automatiquement chargée",
    ...
    "    d'écriture dans l'EEPROM est limit??environ 100 000 cycles Quand",
    "    la condition est fausse, rien n'apparait. Cette instruction doit",
    "    être complétement ?droite dans une séquence.",
    "",
    "",
    ...
    "    écrit un seul caractère vers l'UART. La valeur ASCII du caractère ?,
    "    transmettre doit avoir ét?stock?dans 'var' par avance. La condition",
    ...
    "    vous assurer que le premier caractère ?bien ét?transmis, avant",
    ...
    "    de toutyes les fréquences. Ldmicro vous avertit dans ce cas.",
    "",
    "    Quand la condition d'entrée de cette instruction passe de faux ?vrai,",
    "    elle commence ?envoyer une chaine compléte vers le port série. Si",
    ...
    "    vous devez nécessairement utiliser '\\4' ?la place.",
    "",
    "    Si la variable peut être négative, alors utilisez '\\-3d' (ou `\\-4d'",
    "    etc.) ?la place. Ceci oblige LDmicro ?imprimer un espace d'entête",
    ...
    "",
    "    Utiliser `\\\\' pour l'anti-slash. en addition ?une séquence escape ",
    ...
    "    consomme une grande quantit?de mémoire, elle doit être utilisée",
    ...
    "bien ?l'intérieur d'un entier 16 bits, il doit donc être théoriquement",
    ...
    "   ||-----------{ 1200 / x }-----------||",
    "",
    "Mathématiquement c'est identique, la première donne y = 0 , c'est ?dire",
    ...
    "pratique c'est une mauvaise idée, ?cause de la compléxit?que cela",
    "peut engendrer et plus difficile ?éditer sans effacer et redessiner un",
    "certain nombre d'éléments de logique.",
    "",
    "Néanmoins, c'est une bonne idée de regrouper différents éléments d'un bloc",
    "logique dans une seule séquence. Le code génér?est identique dans les",
    "deux cas, et vous pouvez voir ce que fait la séquence dans le diagramme",
    "?contacts.",
    "",
    "                  *                 *                  *",
    "",
    "En général, il est considér?comme mauvais d'écrire du code dont la",
    ...
    "=======",
    "",
    "LDmicro ne génére pas un code très efficace; il est lent ?exécuter et",
...
char *HelpTextTr[] = {
    ...
    "gerekli kodu üretir. Bu i?için kullanýlabilecek deðiþik programlar vardýr.",
    "Örneðin BASIC, C, assembler gibi. Bu programlar kendi dillerinde yazýlmýþ",
    "programlar?iþlemcilerde çalýþabilecek dosyalar haline getirirler.",
    "",
    ...
    "gibi davranýrlar. Ladder diyagram?ile ilgili bol miktarda kaynak internet",
    "üzerinde bulunmaktadýr. Burada LDMicro'ya has özelliklerden bahsedeceðiz.",
    "",
    "LDmicro ladder diyagramýn?PIC16 veya AVR koduna çevirir. Aþaðýda desteklenen",
    ...
    "",
    "LDMicro ile ladder diyagramýn?çizebilir, devrenizi denemek için gerçek zamanl?",
    "simülasyon yapabilirsiniz. Programýnýzýn çalýþtýðýndan eminseniz programdaki ",
    "giri?ve çýkýþlara mikrokontrolörün bacaklarýn?atarsýnýz. Ýþlemci bacaklar?",
    "belli olduktan sonra programýnýz?derleyebilirsiniz. Derleme sonucunda oluþan",
    "dosya .hex dosyasýdýr. Bu dosyay?PIC/AVR programlayýc?ile iþlemcinize kaydedersiniz.",
    "PIC/AVR ile uðraþanlar konuya yabanc?deðildir.",
    "",
    "",
    "LDMicro ticari PLC programlar?gibi tasarlanmýþtýr. Baz?eksiklikler vardýr. ",
    "Kitapçýðý dikkatlice okumanýz?tavsiye ederim. Kullaným esnasýnda PLC ve ",
    ...
    "gerekli dosyalar?siz saðlamalýsýnýz. Yani, LDMicro sadece PlcCycle()",
    "isimli fonksiyonu üretir. Her döngüde PlcCycle fonksiyonunu çaðýrmak, ve",
    "PlcCycle() fonksiyonunun çaðýrdýð?dijital giriþi yazma/okuma vs gibi",
    "G/?fonksiyonlar?sizin yapmanýz gereken iþlemlerdir.",
    "Oluþturulan kodu incelerseniz faydal?olur.",
    "",
    ...
    "Dosya varsa açýlýr. Yoksa hata mesaj?alýrsýnýz. Ýsterseniz .ld uzantýsýn?,
    "ldmicro.exe ile iliþkilendirirseniz .ld uzantýl?bir dosyay?çift týklattýðýnýzda",
    ...
    "==============",
    "",
    "LDMicro açýldýðýnda bo?bir program ile baþlar. Varolan bir dosya ile baþlatýrsanýz",
    "bu program açýlýr. LDMicro kendi dosya biçimini kullandýðýndan diðer dosya",
    "biçimlerinden dosyalar?açamazsýnýz.",
    "",
    "Bo?bir dosya ile baþlarsanýz ekranda bir tane bo?satýr görürsünüz. Bu satýra",
    "komutlar?ekleyebilir, satýr sayýsýn?artýrabilirsiniz. Satýrlara Rung denilir.",
    ...
    "",
    "    * Xname -- mikrokontrolördeki bir giri?bacaðý",
    "    * Yname -- mikrokontrolördeki bir çýkýþ bacaðý",
    "    * Rname -- `dahili röle': hafýzada bir bit.",
    "    * Tname -- zamanlayýc? turn-on, turn-off yada retentive ",
    "    * Cname -- sayýc? yukar?yada aþaðý sayýc?,
    "    * Aname -- A/D çeviriciden okunan bir tamsay?deðer",
    "    * name  -- genel deðiþken (tamsay?",
    "",
    "Ýstediðiniz ismi seçebilirsiniz. Seçilen bir isim nerede kullanýlýrsa",
    "kullanýlsýn ayn?yere denk gelir. Örnekler; bir satýrda Xasd kullandýðýnýzda",
    "bir baþka satýrda Xasd kullanýrsanýz ayn?deðere sahiptirler. ",
    "Bazen bu mecburi olarak kullanýlmaktadýr. Ancak baz?durumlarda hatal?olabilir.",
    ...
    "",
    "Deðiþken isimleri harfleri, sayýlar? alt çizgileri ihtiva edebilir.",
    "(_). Deðiþken isimleri say?ile baþlamamalýdýr. Deðiþken isimleri büyük-küçük harf duyarlýdýr.",
    "Örneðin; TGec ve Tgec ayn?zamanlayýcýlar deðildir.",
    "",
    "",
    "Genel deðiþkenlerle ilgili komutlar (MOV, ADD, EQU vs) herhangi bir",
    "isimdeki deðiþkenlerle çalýþýr. Bunun anlam?bu komutlar zamanlayýcýlar",
    "ve sayýcýlarla çalýþýr. Zaman zaman bu faydal?olabilir. Örneðin; bir ",
    ...
    "sahip olabilirler) Onluk say?sisteminde say?kullanabilirsiniz. Týrnak",
    ...
    "güncelleþtirilir. Sadece 'Xname', 'Yname', 'Aname' elemanlar?için",
    "mikrokontrolörün bacak numaralar?belirtilmelidir. Önce Ayarlar->Ýþlemci Seçimi",
    "menüsünden iþlemciyi seçiniz. Daha sonra G/?uçlarýn?çift týklatarak açýlan",
    "pencereden seçiminizi yapýnýz.",
    "",
    "Komut ekleyerek veya çýkararak programýnýz?deðiþtirebilirsiniz. Programdaki",
    "kursör eleman eklenecek yeri veya hakkýnda iþlem yapýlacak eleman?göstermek",
    "amacýyla yanýp söner. Elemanlar arasýnda <Tab> tuþu ile gezinebilirsiniz. Yada",
    "eleman?fare ile týklatarak iþlem yapýlacak eleman?seçebilirsiniz. Kursör elemanýn",
    ...
    "Baz?iþlemleri yapamazsýnýz. Örneðin bir bobinin saðýna eleman ekleyemezsiniz.",
    "LDMicro buna izin vermeyecektir.",
    "",
    "Program bo?bir satýrla baþlar. Kendiniz alta ve üste satýr ekleyerek dilediðiniz",
    "gibi diyagramýnýz?oluþturabilirsiniz. Yukarýda bahsedildiði gibi alt devreler",
    "oluþturabilirsiniz.",
    "",
    "Programýnýz yazdýðýnýzda simülasyon yapabilir, .hex dosyasýn?oluþturabilirsiniz.",
    "",
    "SÝMÜLASYON",
    "==========",
    "",
    "Simülasyon moduna geçmek için Simülasyon->Simülasyon modu menüsün?týklatabilir,",
    "yada <Ctrl+M> tu?kombinasyonuna basabilirsiniz. Simülasyon modunda program",
    "farkl?bir görünt?alýr. Kursör görünmez olur. Enerji alan yerler ve elemanlar",
    "parlak kýrmýz? enerji almayan yerler ve elemanlar gri görünür. Boþluk tuþuna",
    "basarak bir çevrim ilerleyebilir yada menüden Simülasyon->Gerçek Zamanl?Simülasyonu Baþlat",
    "diyerek (veya <Ctrl+R>) devaml?bir çevrim baþlatabilirsiniz. Ladder diyagramýnýn",
    "çalýþmasýna göre gerçek zamanl?olarak elemanlar ve yollar program tarafýndan deðiþtirilir.",
    "",
    "Giri?elemanlarýnýn durumunu çift týklatarak deðiþtirebilirsiniz. Mesela, 'Xname'",
    "kontaðýn?çift týklatýranýz açýktan kapalýya veya kapalýdan açýða geçi?yapar.",
    "",
    "DERLEME",
    "=======",
    "",
    "Ladder diyagramýnýn yapýlmasýndaki ama?iþlemciye yüklenecek .hex dosyasýnýn",
    ...
    " 2- G/?uçlarýnýn mikrokontrolördeki hangi bacaklara baðlanacaðý seçilmelidir.",
    "    Elemanýn üzerine çift týklanýr ve çýkan listeden seçim yapýlýr.",
    " 3- Çevrim süresi tanýmlanmalýdýr. Ayarlar->Ýþlemci Ayarlar?menüsünden yapýlýr.",
    "	Bu süre iþlemcinin çalýþtýð?frekansa baðlýdýr. Çoðu uygulamalar için 10ms",
    "	uygun bir seçimdir. Ayn?yerden kristal frekansýn?ayarlamay?unutmayýnýz.",
    "",
    "Artýk kodu üretebilirsiniz. Derle->Derle yada Derle->Farkl?Derle seçeneklerinden",
    "birini kullanacaksýnýz. Aradaki fark Kaydet ve Farkl?Kaydet ile aynýdýr. Sonuçta",
    "Intel IHEX dosyanýz programýnýzda hata yoksa üretilecektir. Programýnýzda hata varsa",
    "uyar?alýrsýnýz.",
    "",
    "Progamlayýcýnýz ile bu dosyay?iþlemcinize yüklemelisiniz. Buradaki en önemli nokta",
    "iþlemcinin konfigürasyon bitlerinin ayarlanmasýdýr. PIC16 iþlemciler için gerekli ",
    "ayar bilgileri hex dosyasýna kaydedildiðinden programlayýcýnýz bu ayarlar?algýlayacaktýr.",
    "Ancak AVR iþlemciler için gerekli ayarlar?siz yapmalýsýnýz.",
    "",
    ...
    "	çýkýþýndaki sinyalde 0 olur. Eðer kontrol eden sinyal 1 olursa çýkýþ?da 1",
    "	olur ve çýkýþa baðl?bobin aktif olur. Bu kontak iþlemci bacaðýndan alýnan",
    "	bir giri? çýkýþ bacaðý yada dahili bir röle olabilir.",
    "",
    "",
    "> KONTAK, NORMALDE KAPALI       Xname           Rname          Yname",
    "                             ----]/[----     ----]/[----    ----]/[----",
    "",
    "    Normalde kapal?bir anahtar gibi davranýr. Komutun kontrol eden sinyal 0 ise",
    "	çýkýþ?1 olur. Eðer kontrol eden sinyal 1 olursa çýkýþ?0 olur ve çýkýþa",
    "	baðl?elemanlar pasif olur. Normalde çýkýþa gerilim verilir, ancak bu kontaðý ",
    "	kontrol eden sinyal 1 olursa kontaðýn çýkýþýnda gerilim olmaz. Bu kontak ",
    "	iþlemci bacaðýndan alýnan bir giri? çýkýþ bacaðý yada dahili bir röle olabilir",
    "",
    "",
    ...
    "	Bobine	giri?deðiþkeni atamak mantýksýzdýr. Bu eleman bir satýrda",
    ...
    "	süre kadar sürede çýkýþ 0 olarak kalýr, süre bitince çýkýþ?1 olur. ",
    "    Giriþindeki sinyal 1'den 0'a geçerse çýkýþ hemen 0 olur.",
    "	Giriþi 0 olduðu zaman zamanlayýc?sýfýrlanýr. Ayrýca; ayarlanan süre",
    "	boyunca giri?1 olarak kalmalýdýr.",
    "",
    "    Zamanlayýc?0'dan baþlayarak her çevrim süresinde 1 artarak sayar.",
    "	Say?ayarlanan süreye eþit yada büyükse çýkýþ 1 olur. Zamanlayýc?,
    ...
    "	süre kadar sürede çýkýþ 1 olarak kalýr, süre bitince çýkýþ?0 olur. ",
    "    Giriþindeki sinyal 0'dan 1'e geçerse çýkýþ hemen 1 olur.",
    "	Giriþi 0'dan 1'e geçtiðinde zamanlayýc?sýfýrlanýr. Ayrýca; ayarlanan",
    "	süre boyunca giri?0 olarak kalmalýdýr.",
    "",
    "    Zamanlayýc?0'dan baþlayarak her çevrim süresinde 1 artarak sayar.",
    "	Say?ayarlanan süreye eþit yada büyükse çýkýþ 1 olur. Zamanlayýc?,
    "	deðiþkeni üzerinde iþlem yapmak mümkündür. (Örneðin MOV komutu ile)",
    ...
    "                               -[RTO 1.000 s]-",
    "",
    "	Bu zamanlayýc?giriþindeki sinyalin ne kadar süre ile 1 olduðunu",
    "	ölçer. Ayaralanan süre boyunca giri?1 ise çýkýþ?1 olur. Aksi halde",
    "	çýkýþ?0 olur. Ayarlanan süre devaml?olmas?gerekmez. Örneðin; süre ",
    "	1 saniyeye ayarlanmýþsa ve giri?önce 0.6 sn 1 olmuþsa, sonra 2.0 sn",
    "	boyunca 0 olmuþsa daha sonra 0.4 sn boyunca giri?tekrar 1 olursa",
    "	0.6 + 0.4 = 1sn olduðundan çýkýþ 1 olur. Çýkýþ 1 olduktan sonra",
    "	giri?0 olsa dahi çýkýþ 0'a dönmez. Bu nedenle zamanlayýc?RES reset",
    ...
    "                             ----{RES}----     ----{RES}----",
    "",
    "    Bu komut bir zamanlayýc?veya sayýcýy?sýfýrlar. TON ve TOF zamanlayýc?,
    "	komutlar?kendiliðinden sýfýrlandýðýndan bu komuta ihtiya?duymazlar.",
    "	RTO zamanlayýcýs?ve CTU/CTD sayýcýlar?kendiliðinden sýfýrlanmadýðýndan",
    "	sýfýrlanmalar?için kullanýc?tarafýndan bu komutile sýfýrlanmas?,
    "	gerekir. Bu komutun giriþi 1 olduðunda sayýc?zamanlayýc?sýfýrlanýr.",
    ...
    "                           --[OSR_/ ]--",
    "",
    "    Bu komutun çýkýþ?normalde 0'dýr. Bu komutun çýkýþýnýn 1 olabilmesi",
    "	için bir önceki çevrimde giriþinin 0 þimdiki çevrimde giriþinin 1 ",
    "	olmas?gerekir. Komutun çýkýþ?bir çevrimlik bir pals üretir.",
    ...
    "                           ----+----+----      ----+     +----",
    "",
    "    Kýsa devrenin çýkýþ?her zaman giriþinin aynýsýdýr.",
    "	Açýk devrenin çýkýþ?her zaman 0'dýr. Bildiðimiz açýk/kýsa devrenin",
    ...
    "	kontrol edilmesi gerektiðinde paralel baðlant?yapmak gerekir. Bu ise zordur.",
    "	Bu iþlemi kolayca yapabilmek için ana kontrol rölesini kullanabiliriz.",
    "	Ana kontrol rölesi eklendiðinde kendisinden sonraki satýrlar bu röleye baðl?,
    "	hale gelir. Böylece; birden fazla satýr tek bir þart ile kontrol?saðlanýr.",
    ...
    "	baþlangýc?ikincisi ise bitiþi temsil eder. Ana kontrol rölesi kullandýktan",
    "	sonra iþlevini bitirmek için ikinci bir ana kontrol rölesi eklemelisiniz.",
    "",
    "> MOVE                      {destvar :=  }      {Tret :=     }",
    "                           -{ 123     MOV}-    -{ srcvar  MOV}-",
    "",
    "    Giriþi 1 olduðunda verilen sabit sayýy?(123 gibi) yada verilen deðiþkenin",
    "	içeriðini (srcvar) belirtilen deðiþkene (destvar) atar. Giri?0 ise herhangi",
    "	bir iþlem olmaz. Bu komut ile zamanlayýc?ve sayýcýlar da dahil olmak üzere",
    "	tüm deðiþkenlere deðer atayabilirsiniz. Örneðin Tsay zamanlayýcýsýna MOVE ile",
    "	0 atamak ile RES ile sýfýrlamak ayn?sonucu doðurur. Bu komut bir satýrýn",
    ...
    "	iþlemin sonucunu kaydeder. Ýþlenen bilgi zamanlayýc?ve sayýcýlar dahil",
    "	olmak üzere deðiþkenler yada sabit sayýlar olabilir. Ýþlenen bilgi 16 bit",
    "	iþaretli sayýdýr. Her çevrimde iþlemin yeniden yapýldýð?unutulmamalýdýr.",
    "	Örneðin artýrma yada eksiltme yapýyorsanýz yükselen yada düþen kenar",
    "	kullanmanýz gerekebilir. Bölme (DIV) virgülden sonrasýn?keser. Örneðin;",
    ...
    "                       -[ var2 ]-     -[ vartwo]-     -[ Cup]-",
    "",
    "    Deðiþik karþýlaþtýrma komutlar?vardýr. Bu komutlarýn giriþi doðru (1)",
    "	ve verilen þart da doðru ise çýkýþlar?1 olur.",
    "",
    "",
    "> SAYICI                       Cname          Cname",
    "                           --[CTU >=5]--  --[CTD >=5]--",
    ...
    "	çýkýþlar?aktif (1) olur. CTU ve CTD sayýcýlarýna ayn?ismi erebilirsiniz.",
    "	Böylece ayn?sayýcýy?artýrmýþ yada eksiltmi?olursunuz. RES komutu sayýcýlar?,
    "	sýfýrlar. Sayýcýlar ile genel deðiþkenlerle kullandýðýnýz komutlar?kullanabilirsiniz.",
    "",
    "",
    "> DAÝRESEL SAYICI              Cname",
    "                           --{CTC 0:7}--",
    "",
    "    Normal yukar?sayýcýdan fark?belirtilen limite ulaþýnca sayýc?tekrar 0'dan baþlar",
    "    Örneðin sayýc?0, 1, 2, 4, 5, 6, 7, 0, 1, 2, 3, 4, 5, 6, 7, 0, 2,.... þeklinde",
    "	sayabilir. Yani bir dizi sayýc?olarak düþünülebilir. CTC sayýcýlar giriþlerinin",
    ...
    "	sayýsýn?3 olarak tanýmladýysanýz reg0, reg1, reg2 deðikenleri ile çalýþýrsýnýz.",
    ...
    "	Geni?bir kaydedici hafýzada çok yer kaplar.",
    ...
    "	`dest' tamsay?deðiþkeni `i' tamsay?deðiþkenine karþýlýk gelen deðeri alýr. Sýra",
    "	0'dan baþlar. bu nedenle `i' 0 ile (n-1) arasýnda olabilir. `i' bu deðerler ",
    "	arasýnda deðilse komutun ne yapacaðý tanýml?deðildir.",
    "	Bu komut bir satýrýn saðýndaki en son komut olmalýdýr.",
    "",
    "",
    ...
    "                           -{ PWL[xvar]  }-",
    "",
    "    Bir matris tablo olarak düþünülebilir. Bir deðere baðl?olarak deðerin önceden",
    ...
    "",
    "    Farzedelimki x tamsay?giri?deðerini y tamsay?çýkýþ deðerine yaklaþtýrmak ",
    ...
    "        (x3, y3)   = (100, 100)",
    "",
    "    Dört deðeri parçal?lineer tabloya gireriz. Komut, xvar'ýn deðerine bakarak",
    "	yvar'a deðer verir. Örneðin, yukarýdaki örneðe bakarak, xvar = 10 ise",
    "	yvar = 50 olur.",
    "    ",
    "	Tabloya kayýtl?iki deðerin arasýnda bir deðer verirseniz verilen deðer de",
    "	alýnmas?gereken iki deðerin arasýnda uygun gelen yerde bir deðer olur.",
    ...
    "	nedenle 55 ve 75 deðerlerinin ortas?olan 75 deðeri alýnýr.",
    "	",
    "	Deðerler x koordinatýnda artan deðerler olarak yazýlmalýdýr. 16 bit tamsay?,
    "	kullanan baz?deðerler için arama tablosu üzerinde matematik iþlemler",
    ...
    "        (x1, y1)    = (300, 300)",
    "",
    "    Bu tip hatalar?noktalar arsýnda ara deðerler oluþturarak giderebilirsiniz.",
    "	Örneðin aþaðýdaki tablo yukarýdakinin aynýs?olmasýna raðmen hata ",
    "	oluþturmayacaktýr.",
    "    ",
    "        (x0, y0)    = (  0,   0)",
    "        (x1, y1)    = (150, 150)",
    ...
    "	Daha fazla nokta demek daha fazla kod ve daha yava?çalýþma demektir.",
    ...
    "	deðerinden daha küçük bir deðer girmenin sonucu tanýml?deðildir.",
    "	Bu komut bir satýrýn saðýndaki en son komut olmalýdýr.",
    "",
    "> A/D ÇEVÝRÝCÝDEN OKUMA        Aname",
    "                           --{READ ADC}--",
    "",
    "    LDmicro A/D  çeviriciden deðer okumak için gerekli kodlar?desteklediði",
    ...
    "	ADC olmayan bacaklarýn tanýmlanmasýn?LDMicro engelleyecektir.",
    "	Bu komut bir satýrýn saðýndaki en son komut olmalýdýr.",
    "",
    "	> PWM PALS GENÝÞLÝÐ?AYARI      duty_cycle",
    "                             -{PWM 32.8 kHz}-",
    "",
    "    LDmicro desteklediði mikrokontrolörler için gerekli PWM kodlarýn?üretebilir.",
    ...
    "	geniþliðin yüzde kaçýnýn palsi oluþturacaðý ayarlanýr. 0 periyodun tüm?sýfýr",
    "	100 ise periyodun tamam?1 olsun anlamýna gelir. 10 deðeri palsin %10'u 1 geri",
    "	kalan %90'?sýfýr olsun anlamýna gelir.",
    "",
    "    PWM frekansýn?ayarlayabilirsiniz. Verilen deðer Hz olarak verilir.",
    ...
    "	kullanýlýr. Bu nedenle PWM en az iki tane zamanlayýcýs?olan iþlemcilerde kullanýlýr.",
    ...
    "                           --{PERSIST}--",
    "",
    "    Bu komut ile belirtilen deðiþkenin EEPROM'da saklanmas?gereken bir deðiþken olduðunu",
    "	belirmi?olursunuz. Komutun giriþi doðru ise belirtilen deðiþkenin içeriði EEPROM'a",
    "	kaydedilir. Enerji kesildiðinde kaybolmamas?istenen deðerler için bu komut kullanýlýr.",
    ...
    "	Ayrýca bir iþlem yapýlmas?gerekmez.",
    "	Bu komut bir satýrýn saðýndaki en son komut olmalýdýr.",
    "",
    "************************",
    "> UART (SER?BÝLG? AL          var",
    "                           --{UART RECV}--",
    "",
    "    LDmicro belirli iþlemciler için gerekli UART kodlarýn?üretebilir. AVR iþlemcilerde",
    "	sadece UART1 (UART0) deðil) desteklenmektedir. Ýletiþim hýz?(baudrate) ayarlarýn?",
    "	Ayarlar->Ýþlemci Ayarlar?menüsünden yapmalýsýnýz. Hýz kristal frekansýna baðl?olup,",
    "	baz?hýzlar desteklenmeyebilir. Bu durumda LDMicro sizi uyaracaktýr. ",
    "    ",
    "    Bu komutun giriþi yanlýþsa herhangi bir iþlem yapýlmaz. Doðru ise UART'dan 1 karakter",
    "	alýnmaya çalýþýlýr. Okuma yapýlamaz ise komutun çýkýþ?yanlýþ (0) olur. Karakter",
    "	okunursa okunan karakter `var' deðiþkeninde saklanýr ve komutun çýkýþ?doðru (1) olur.",
    "	Çýkýþýn doðru olmas?sadece bir PLC çevrimi sürer.",
    "",
    "",
    "> UART (SER?BÝLG? GÖNDER      var",
    "                           --{UART SEND}--",
    "",
    "    LDmicro belirli iþlemciler için gerekli UART kodlarýn?üretebilir. AVR iþlemcilerde",
    "	sadece UART1 (UART0) deðil) desteklenmektedir. Ýletiþim hýz?(baudrate) ayarlarýn?",
    ...
    "	",
    "    Bu komutun giriþi yanlýþsa herhangi bir iþlem yapýlmaz. Doðru ise UART'dan 1 karakter",
    "	gönderilir. Gönderilecek karakter gönderme iþleminden önce `var' deðiþkeninde sakl?,
    "	olmalýdýr. Komutun çýkýþ?UART meþgulse (bir karakterin gönderildiði sürece)",
    ...
    "",
    "    Bu komut yerine biçimlendirilmi?kelime komutunu (bir sonraki komut) inceleyiniz.",
    "	Biçimlendirilmi?kelime komutunun kullaným?daha kolaydýr. Ýstediðiniz iþlemleri",
    ...
    "",
    "    Deðiþken negatif bir say?olabilecekse `\\-3d' (veya `\\-4d') gibi uygun bir deðer",
    ...
    "",
    "    Ayn?anda birka?iþlem tanýmlanýrsa, yada UART ile ilgili iþlemler birbirine",
    "	karýþýk hale getirilirse programýn davranýþ?belirli olmayacaktýr. Bu nedenle",
    "	dikkatli olmalýsýnýz.",
    "",
    "    Kullanýlabilecek özel karakterler (escape kodlar? þunlardýr:",
    "        * \\r   -- satýr baþýna ge?,
    "        * \\n   -- yeni satýr",
    "        * \\f   -- kaðýd?ilerlet (formfeed)",
    "        * \\b   -- bir karakter geri gel (backspace)",
    "        * \\xAB -- ASCII karakter kodu 0xAB (hex)",
    "",
    "    Bu komutun çýkýþ?bilgi gönderiyorken doðru diðer durumlarda yanlýþ olur.",
    "    Bu komut program hafýzasýnda çok yer kaplar.",
    "",
    "",
    "MATEMATÝKSEL ÝÞLEMLER ÝLE ÝLGÝL?BÝLG?,
    "======================================",
    "",
    "Unutmayýn ki, LDMicro 16-bit tamsay?matematik komutlarýna sahiptir.",
    "Bu iþlemlerde kullanýlan deðerler ve hesaplamanýn sonucu -32768 ile",
    "32767 arasýnda bir tamsay?olabilir.",
    "",
    "Mesela y = (1/x)*1200 formülün?hesaplamaya çalýþalým. x 1 ile 20",
    "arasýnda bir sayýdýr. Bu durumda y 1200 ile 60 arasýnda olur. Bu say?,
    "16-bit bir tamsay?sýnýrlar?içindedir. Ladder diyagramýmýz?yazalým.",
    ...
    "yanlýþ sonu?verecektir. (y=0 olur). Bu hata `temp' deðiþkeninin 1'den",
    "küçük sonu?vermesindendir.Mesela x = 3 iken (1 / x) = 0.333 olur. Ama",
    "0.333 bir tamsay?deðildir. Bu nedenle sonu?0 olur. Ýkinci adýmda ise",
    "y = temp * 1200 = 0 olur. Ýkinci þekilde ise bölen bir tamsay?olduðundan",
    "sonu?doðru çýkacaktýr.",
    "",
    ...
    "32767 sýnýr?aþýlmýþ olacaktýr. ",
    "",
    "Hesaplamalarýnýzda mantýksal deðiþimler yaparak doðru sonuçlar elde edebilirsiniz.",
    "Örneðin; y = 1.8*x ise formülünüz?y = (9/5)*x þeklinde yazýnýz.(1.8 = 9/5)",
    "y = (9*x)/5 þeklindeki bir kod sonucu daha tutarl?hale getirecektir.",
    ...
    "==============",
    "",
    "Programýn saðladýð?kolaylýklardan faydalanýn. Mesela:",
    "",
    "   ||       Xa               Ya        ||",
    " 1 ||-------] [--------------( )-------||",
    "   ||                                  ||",
    ...
    "mantýksýz bir programlama yapýlmýþtýr. Çünk?hem Xa hemde Xb ayn?,
    "anda doðru olabilir.",
    "",
    "   ||       Xa         {v  :=       }  ||",
    " 1 ||-------] [--------{ 12      MOV}--||",
    ...
    "iþlem 4-bit binary say?tamsayýya çevrilmektedir.",
    "",
    "   ||                                   {v  :=       }  ||",
    " 3 ||-----------------------------------{ 0       MOV}--||",
    "   ||                                                   ||",
    ...
    "=============",
    "",
    "LDmicro tarafýndan üretilen kodlar çok verimli kodlar deðildir. Yava?çalýþan",
    "ve hafýzada fazla yer kaplayan kodlar olabilirler. Buna raðmen orta büyüklükte",
    "bir PIC veya AVR küçük bir PLC'nin yaptýð?iþi yapar. Bu nedenle diðer sorunlar",
    "yer yer gözard?edlebilir.",
    "",
    "Deðiþken isimleri çok uzun olmamalýdýr. ",
    "",
    "Programýnýz yada kullandýðýnýz hafýza seçtiðiniz iþlemcinin sahip olduðundan",
    "büyükse LDMicro hata vermeyebilir. Dikkat etmezseniz programýnýz hatal?çalýþacaktýr.",
    "",
    "Bulduðunuz hatalar?yazara bildiriniz.",
    "",
    "Teþekkürler:",
    "    * Marcelo Solano, Windows 98'deki UI problemini bildirdiði için,",
    "    * Serge V. Polubarjev, PIC16F628 iþlemcisi seçildiðinde RA3:0'ýn çalýþmadýð?,
    "	  ve nasýl düzelteceðimi bildirdiði için,",
    "    * Maxim Ibragimov, ATmega16 ve ATmega162 iþlemcileri test ettikleri, problemleri",
    "	  bulduklar?ve bildirdikleri için,",
    "    * Bill Kishonti, sýfýra bölüm hatas?olduðunda simülasyonun çöktüðün?bildirdikleri",
    "	  için,",
    "    * Mohamed Tayae, PIC16F628 iþlemcisinde EEPROM'da saklanmas?gereken deðiþkenlerin",
    "	  aslýnda saklanmadýðýn?bildirdiði için,",
    "    * David Rothwell, kullanýc?arayüzündeki birka?problemi ve \"Metin Dosyas?Olarak Kaydet\"",
    ...
    "kaynak kodda deðiþiklik yapabilirsiniz. Programýn kullaným?Free Software Foundation",
    "tarafýndan yazýlan GNU General Public License (version 3 ve sonras?þartlarýna baðlýdýr.",
    "",
    "Program faydal?olmas?ümidiyle daðýtýlmýþtýr. Ancak hiçbir garanti verilmemektedir.",
    "Detaylar için  GNU General Public License içeriðine bakýnýz.",
    "",
    "Söz konusu sözleþmenin bir kopyas?bu programla beraber gelmi?olmas?gerekmektedir.",
