...
void UpdateMainWindowTitleBar(void)
{
    ...
    if(InSimulationMode) {
        if(RealTimeSimulationRunning) {
            strcpy(line, _("LDmicro86 Ladder - Simulation (Running)"));
        } else {
            strcpy(line, _("LDmicro86 Ladder - Simulation (Stopped)"));
        }
    } else {
        strcpy(line, _("LDmicro86 Ladder - Program Editor"));
    }
...
HMENU MakeMainWindowMenus(void)
{
	...
	*/
	...
	// vic coment Compile button
    // AppendMenu(compile, MF_STRING, MNU_COMPILE, _("&Compile\tF5"));
    AppendMenu(compile, MF_STRING, MNU_COMPILE_AS, _("Compile &As...\tF5"));
